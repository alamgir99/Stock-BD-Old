...
function read_last_snap()
{
    ...
    $name =  SBD_ROOT.'/prv/temp/gqvoloff.txt';   
...
function get_price_table_gq()
{
    ...
    $cmd = 'curl http://www.gqsecurities.com/latest_share_price.asp --connect-timeout 25 --max-time 45 -o ' .$fname; 
    $err_code = exec ($cmd);
    ...
    foreach($dataarray as $data){
            ...
            // this is a hack for GS to avoid untraded vol
            
            
            if($firstsnap){
                $offset =  trim($vals[8])-trim($vals[11]);
                if($offset == 0) {
                    $vol =  0;
                    $gqsvoloff[$ticker] = trim($vals[11]);
                }
                else
                    $vol =  trim($vals[8]);   
            ...
            $vol = trim($vals[8]) - $gqsvoloff[$ticker];
...
function find_delta()
{
 ...
 function save_data()
 {
    ...
    if($firstsnap) { // save the GQ vol diff
         $name =  SBD_ROOT.'/prv/temp/gqvoloff.txt';   
         $fid =  fopen($fname, "w");
         if($fid === false){
            log_err(__FILE, "Error creating file ".$fname);
            exit(0);
         }
       
       foreach($gqsvoloff as $key => $off){             
            fprintf($fid, $key.','.$off);
        fprintf($fid, "\n");
    }  
    fclose($fid);
    ...
    $totvol = 0;
    foreach($snapshot_del as $tick => $del)
    {
      $totval += $del['vl'];
    }
    
    if($totvol==0){ // data is NOT updating
             log_err(__FILE__, "Vol delta is zero. Data not updating.");
    }
