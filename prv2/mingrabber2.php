...
//find the delta
 ...
 {                              // 5, 10, 15, 20, 25, 30, 35 etc
     ...
     foreach($snap5 as $tick => $vals) {
         $snap[$tick]['op'] = $snap1[$tick]['lp']; // ltp of first snap is the open price of 5-min candle
         $snap[$tick]['cl'] = $snap5[$tick]['lp']; // ltp of fifth snap is the close price of 5-min candle
         //an array of ltps of last 5 snaps
         $ltps = array($snap1[$tick]['lt'], $snap2[$tick]['lt'], $snap3[$tick]['lt'], $snap4[$tick]['lt'],$snap5[$tick]['lt']);
         $snap[$tick]['hi'] = max($ltps);                     // max of ltp of snap 1 thru 5
         $snap[$tick]['lo'] = min($ltps);                     // min of ltp of snap 1 thru 5
         $snap[$tick]['vl'] = $snap1[$tick]['vl']+$snap2[$tick]['vl']
                             +$snap3[$tick]['vl']+$snap4[$tick]['vl']+$snap5[$tick]['vl']; // sum of all 5 snaps
     }
  
     //save the candles
     $fname =  SBD_ROOT.'/prv/temp/candle-'.sprintf("%03d",$snap_no).'.csv';   
     $fid = fopen($fname, "w");
     if($fid){
         foreach($snap as $tick => $vals){
             fwrite($fid, $tick.','.$date_sig.','.$time_sig.','.$vals['op'].','.$vals['hi'].','.$vals['lo'].','.$vals['cl'].','.$vals['vl']);
             fwrite($fid, "\n");
         }
         fclose($fid);
     }
